...
class DatabaseManager:
    ...
    async def initialize_db(self):
        ...
        try:
            ...
            await self.db.group_members.create_index([("chat_id", ASCENDING), ("user_id", ASCENDING)], unique=True)
    ...
    def _node_doc_to_dict(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        """Convierte documento de nodo a diccionario"""
        if not doc:
            return {}
        return {
            "id": doc.get("node_id"),
    ...
    def _group_doc_to_tuple(self, doc: Dict[str, Any]) -> Tuple[Any, ...]:
        """Convierte documento de grupo a tupla"""
        return (
            doc.get("chat_id"),
            doc.get("title"),
            doc.get("type"),
    ...
    async def get_all_settings(self):
        ...
        settings = {}
        async for d in docs:
            settings[d["setting_name"]] = d.get("setting_value")
        return settings
    ...
    async def add_group(self, chat_id, title, chat_type, added_by, username, name, member_count, is_forum: bool = False):
        ...
        # Configuración de bienvenida por defecto
    ...
    async def set_group_welcome_thread(self, chat_id: int, thread_id: Optional[int]):
        """Establece el hilo de bienvenida para un grupo"""
        await self.db.groups.update_one(
            {"chat_id": chat_id},
    ...
    async def clear_group_welcome_thread(self, chat_id: int):
        """Limpia el hilo de bienvenida de un grupo"""
        await self.set_group_welcome_thread(chat_id, None)

    async def get_group_welcome_thread(self, chat_id: int) -> Optional[int]:
        """Obtiene el hilo de bienvenida de un grupo"""
        doc = await self.db.groups.find_one({"chat_id": chat_id}, {"welcome_thread_id": 1})
        return doc.get("welcome_thread_id") if doc else None

    # Tracking de miembros
    async def is_new_member(self, chat_id: int, user_id: int) -> bool:
        """Verifica si un usuario es nuevo en el grupo"""
        existing = await self.db.group_members.find_one({"chat_id": chat_id, "user_id": user_id})
        return existing is None

    async def mark_member_as_seen(self, chat_id: int, user_id: int):
        """Marca a un usuario como ya visto en el grupo"""
        await self.db.group_members.update_one(
            {"chat_id": chat_id, "user_id": user_id},
            {"$set": {"first_seen": datetime.utcnow().isoformat()}},
            upsert=True
        )

    # Welcome settings
    async def get_welcome_settings(self, chat_id):
        """Obtiene configuración de bienvenida de un grupo"""
    ...
    async def set_welcome_mode(self, chat_id: int, mode: str):
        """Establece el modo de bienvenida: 'always' o 'new_only'"""
        await self.db.welcome_settings.update_one(
            {"chat_id": chat_id}, 
            {"$set": {"welcome_mode": mode}}
        )

    async def get_welcome_mode(self, chat_id: int) -> str:
        """Obtiene el modo de bienvenida"""
        doc = await self.db.welcome_settings.find_one({"chat_id": chat_id}, {"welcome_mode": 1})
        return doc.get("welcome_mode", "always") if doc else "always"

    async def toggle_welcome_status(self, chat_id):
        """Cambia el estado de la bienvenida (activado/desactivado)"""
        doc = await self.db.welcome_settings.find_one({"chat_id": chat_id})
    ...
    async def update_welcome_stats(self, chat_id):
        """Actualiza estadísticas de bienvenida"""
        await self.db.stats.update_one(
            {"chat_id": chat_id},
    ...
    async def ensure_root_node(self, chat_id):
        """Asegura que existe un nodo raíz para el grupo"""
        doc = await self.db.welcome_nodes.find_one({"chat_id": chat_id, "parent_id": None})
        if doc:
            return int(doc["node_id"])

        # Crear nodo raíz basado en configuración existente
    ...
    async def get_root_node(self, chat_id):
        """Obtiene el nodo raíz de un grupo"""
        doc = await self.db.welcome_nodes.find_one({"chat_id": chat_id, "parent_id": None})
        if not doc:
            await self.ensure_root_node(chat_id)
            doc = await self.db.welcome_nodes.find_one({"chat_id": chat_id, "parent_id": None})
    ...
    async def update_node_image(self, node_id: int, image_url):
        """Actualiza la imagen de un nodo"""
    ...
    async def update_node_parse_mode(self, node_id: int, parse_mode: str):
        """Actualiza el modo de parseo de un nodo"""
    ...
    async def get_node_buttons(self, node_id: int):
        """Obtiene los botones de un nodo"""
    ...
    async def delete_node_recursive(self, node_id: int):
        ...
        if not node or node['parent_id'] is None:
            return  # No eliminar nodo raíz

        # Eliminar nodos hijos recursivamente
        children = await self.get_child_nodes(node['chat_id'], node_id)
        for child in children:
            await self.delete_node_recursive(child['id'])

        # Remover botones que apuntan a este nodo
